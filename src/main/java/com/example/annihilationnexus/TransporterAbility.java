...
public class TransporterAbility {
    ...
    private static final Map<UUID, Map<Location, Location>> activePortals = new HashMap<>(); // Player UUID -> {Portal
                                                                                             // Location -> Linked
                                                                                             // Location}
    ...
    static {
        unsuitableMaterials = java.util.stream.Stream.of(
                ...
                Material.getMaterial("TALL_GRASS"), Material.getMaterial("GRASS"), Material.getMaterial("FERN"),
                Material.getMaterial("LARGE_FERN"),
                Material.getMaterial("DANDELION"), Material.getMaterial("POPPY"), Material.getMaterial("BLUE_ORCHID"),
                Material.getMaterial("ALLIUM"), Material.getMaterial("AZURE_BLUET"),
                Material.getMaterial("RED_TULIP"), Material.getMaterial("ORANGE_TULIP"),
                Material.getMaterial("WHITE_TULIP"), Material.getMaterial("PINK_TULIP"),
                Material.getMaterial("OXEYE_DAISY"), Material.getMaterial("CORNFLOWER"),
                Material.getMaterial("LILY_OF_THE_VALLEY"), Material.getMaterial("WITHER_ROSE"),
                Material.getMaterial("SUNFLOWER"), Material.getMaterial("LILAC"), Material.getMaterial("ROSE_BUSH"),
                Material.getMaterial("PEONY"),
                Material.getMaterial("SUGAR_CANE"), Material.getMaterial("CACTUS"), Material.getMaterial("KELP"),
                Material.getMaterial("SEAGRASS"), Material.getMaterial("TALL_SEAGRASS"),
                Material.getMaterial("VINE"), Material.getMaterial("LADDER"), Material.getMaterial("TRIPWIRE"),
                Material.getMaterial("STRING"),
                Material.getMaterial("TORCH"), Material.getMaterial("WALL_TORCH"),
                Material.getMaterial("REDSTONE_TORCH"), Material.getMaterial("REDSTONE_WALL_TORCH"),
                Material.getMaterial("LEVER"), Material.getMaterial("STONE_BUTTON"),
                Material.getMaterial("WOODEN_BUTTON"),
                Material.getMaterial("RAIL"), Material.getMaterial("POWERED_RAIL"),
                Material.getMaterial("DETECTOR_RAIL"), Material.getMaterial("ACTIVATOR_RAIL"),
        ...
        inventoryBlocks = java.util.stream.Stream.of(
                ...
                Material.HOPPER, Material.DROPPER, Material.DISPENSER, Material.CRAFTING_TABLE,
    ...
    public void handlePortalCreation(Player player, Block clickedBlock) {
        ...
        if (plugin.getPlayerClassManager().isPlayerInPostDeathPortalCooldown(player.getUniqueId())) {
            player.sendMessage(
                    ChatColor.RED + "You cannot create portals after death until you respawn or re-select your class.");
            return;
        }
        ...
        // Check if the block is a Nexus
        if (plugin.getNexusManager().getNexusAt(blockLocation) != null) {
            player.sendMessage(ChatColor.RED + "You cannot place a portal on a Nexus!");
            return;
        }

        // Check placement restrictions
        if (!plugin.getClassRegionManager().isLocationInAllowedRegion(blockLocation)) { // Changed to
                                                                                        // isLocationInAllowedRegion to
                                                                                        // match logic
            player.sendMessage(ChatColor.RED + "You cannot place portals in class command restricted areas.");
            return;
        }
        if (blockLocation.getBlock().getType() == Material.WATER
        ...
        if (unsuitableMaterials.contains(clickedBlock.getType())) {
            player.sendMessage(ChatColor.RED + "You cannot place portals on "
                    + clickedBlock.getType().name().toLowerCase().replace("_", " ") + ".");
            return;
        }
        ...
        if (!playerPortalSelections.containsKey(player.getUniqueId())) {
            ...
            player.sendMessage(
                    ChatColor.YELLOW + "Portal position 1 set. Right-click another block to set position 2.");
            player.playSound(player.getLocation(), Sound.ENTITY_BLAZE_HURT, 1.0f, 1.0f); // Blaze damage sound

            // Immediately change the block to NETHER_QUARTZ_ORE and set metadata for the
            // first portal
            ...
            if (pos1.distance(pos2) > plugin.getTpDistanceLimit()) {
                ...
                player.sendMessage(
                        ChatColor.RED + "Portals are too far apart! Max distance: " + plugin.getTpDistanceLimit());
            ...
            if (heldItem.getType() == plugin.getTransporterItem().getType()) { // Only modify if it's the Transporter
                                                                               // item
    ...
    private void createLinkedPortals(Player owner, Location loc1, Location loc2) {
        ...
        // For the second block (block2), save its original data and set it as a portal
        // block linked to loc1.
        ...
        block1.setMetadata(PORTAL_LINK_METADATA_KEY,
                new FixedMetadataValue(plugin, LocationUtil.locationToString(loc2)));
    ...
    private void setPortalMetadata(Block block, UUID ownerUUID, Location linkedLocation) {
        ...
        if (linkedLocation != null) { // Add null check
            ...
            block.setMetadata(PORTAL_LINK_METADATA_KEY,
                    new FixedMetadataValue(plugin, LocationUtil.locationToString(linkedLocation)));

        }

        block.setMetadata(PORTAL_ORIGINAL_BLOCK_DATA_KEY,
                new FixedMetadataValue(plugin, block.getBlockData().getAsString())); // Store original block data
        ...
        BukkitTask task = new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (player.isSneaking() && player.getLocation().getBlock().getRelative(0, -1, 0).equals(portalBlock)) {
                    ...
                    player.sendMessage(
                            ChatColor.RED + "Teleportation cancelled: You stopped sneaking or moved off the portal.");
    ...
    public void teleportPlayer(Player player, Block portalBlock) {
        ...
        if (linkedLocation != null) {
            ...
            if (!feetBlock.isPassable() || !headBlock.isPassable() ||
                ...
                player.sendMessage(
                        ChatColor.RED + "Teleportation failed: The destination is obstructed or filled with liquid!");
                player.playSound(player.getLocation(), Sound.BLOCK_NOTE_BLOCK_BASS, 1.0f, 1.0f);
                plugin.getLogger().warning("Teleportation for " + player.getName() + " to " + destination
                        + " was cancelled due to obstruction or liquid.");
                return;
            }
            // --- End Safety Check ---
        ...
        if (linkedLocation != null) {
            ...
            if (isPortalBlock(linkedBlock)) { // Check if it's still a portal before destroying
                ...
                plugin.getLogger().warning(
                        "Linked block is not a portal (or was already destroyed): " + linkedBlock.getLocation());
    ...
    private void spawnPortalParticles(Location loc, Particle particle) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                loc.getWorld().spawnParticle(particle, loc.getX() + 0.5, loc.getY() + 1.0, loc.getZ() + 0.5, 5, 0.2,
                        0.2, 0.2, 0.01);
    ...
    // Overload for stopping all continuous particle effects for a player (e.g., on
    // death or new portal creation)
        ...
        for (ItemStack item : player.getInventory().getContents()) {
            ...
            if (item != null && item.getType() == itemType && item.hasItemMeta()) {
                ...
                if (meta.hasDisplayName()) {
                    ...
                    if (displayName.equals(ChatColor.GREEN + "Transporter" + ChatColor.GRAY + " - Entrance Placed")
                            || displayName
                                    .equals(ChatColor.GREEN + "Transporter" + ChatColor.GRAY + " - Teleport Up!")) {
    ...
    // Public static method to destroy a portal and revert its blocks, even if the
    // owner's ability instance is not active
    public static void forceDestroyPortal(AnnihilationNexus plugin, Block block) {
        ...
        if (block.hasMetadata(PORTAL_ORIGINAL_BLOCK_DATA_KEY)) {
            ...
            plugin.getLogger().warning(
                    "No original block data found for portal at " + block.getLocation() + ". Reverting to AIR.");
        }
