...
public class TransporterAbility {
    ...
    private static final String PORTAL_LINK_METADATA_KEY = "TransporterPortalLink";
        ...
        static {
            unsuitableMaterials = java.util.stream.Stream.of(
                    ...
                    Material.OBSIDIAN, Material.BEACON, Material.CRAFTING_TABLE, // Added unsuitable blocks
        ...
        public void handlePortalCreation(Player player, Block clickedBlock) {
            ...
            if (!playerPortalSelections.containsKey(player.getUniqueId())) {
                ...
                if (heldItem.getType() == plugin.getTransporterItem().getType()) {
                    ...
                    if (meta != null) {
                        meta.setDisplayName("Portal Maker - Entrance Placed");
                ...
                if (heldItem.getType() == plugin.getTransporterItem().getType()) { // Only modify if it's the Transporter item
                    ItemMeta meta = heldItem.getItemMeta();
                    if (meta != null) {
                        meta.setDisplayName("Portal Maker - Teleport Up!");
                        heldItem.setItemMeta(meta);
                    }
                }
            }
            ...
            private void createLinkedPortals(Player owner, Location loc1, Location loc2) {
                ...
                // We must NOT call setPortalMetadata again, as that would save the quartz ore's BlockData,
                // overwriting the original block's data that was saved on the first click.
                block1.setMetadata(PORTAL_LINK_METADATA_KEY, new FixedMetadataValue(plugin, loc2.serialize()));
        ...
        private void setPortalMetadata(Block block, UUID ownerUUID, Location linkedLocation) {
            ...
            if (linkedLocation != null) { // Add null check
                block.setMetadata(PORTAL_LINK_METADATA_KEY, new FixedMetadataValue(plugin, linkedLocation.serialize()));
            }
    ...
    public static Location getLinkedPortalLocation(AnnihilationNexus plugin, Block block) {
        if (isPortalBlock(block)) {
            ...
            if (!values.isEmpty()) {
                Object rawValue = values.get(0).value();
                plugin.getLogger().info("Raw metadata value for PORTAL_LINK_METADATA_KEY: " + rawValue.toString() + " (Type: " + rawValue.getClass().getName() + ")");
                if (rawValue instanceof Map) {
                    return Location.deserialize((Map<String, Object>) rawValue);
                } else {
                    plugin.getLogger().warning("PORTAL_LINK_METADATA_KEY raw value is not a Map for deserialization: " + rawValue.toString());
                }
            }
        }
        return null;
    ...
    public void teleportPlayer(Player player, Block portalBlock) {
        ...
        plugin.getLogger().info("Owner check skipped. All players should be able to use this portal.");
    ...
    public void destroyPortal(Block block) {
        ...
        // Destroy the linked portal first
        if (linkedLocation != null) {
            Block linkedBlock = linkedLocation.getBlock();
            if (isPortalBlock(linkedBlock)) {
                ...
                plugin.getLogger().warning("Linked block is not a portal: " + linkedBlock.getLocation());
            }
        }
        ...
        for (ItemStack item : player.getInventory().getContents()) {
            if (item != null && item.getType() == itemType && item.hasItemMeta()) {
                ...
                if (meta.hasDisplayName()) {
                    ...
                    if (displayName.equals("Portal Maker - Entrance Placed") || displayName.equals("Portal Maker - Teleport Up!")) {
